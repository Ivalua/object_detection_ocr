...
class Dataset:
    ...
    def __init__(self, batch_size=3, input_dim=150, **kwargs):
        ...
        self.train = train_datagen.flow_from_directory('/sharedfiles/dogs_vs_cats/train',
                                                     target_size=(self.img_w, self.img_h),
                                                     batch_size=batch_size,
                                                     class_mode='categorical', classes=self.classes)

        self.val = test_datagen.flow_from_directory('/sharedfiles/dogs_vs_cats/validation',
                                                     target_size=(self.img_w, self.img_h),
                                                batch_size=batch_size,
                                                class_mode='categorical', classes=self.classes)

        self.test = test_datagen.flow_from_directory('/sharedfiles/dogs_vs_cats/validation',
                                                     target_size=(self.img_w, self.img_h),
                                                batch_size=batch_size,
                                                class_mode='categorical', classes=self.classes)
